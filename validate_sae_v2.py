...
def get_activations(model, texts, hook_name):
    ...
    for text in texts:
        ...
        tok_no_bos = tokens.squeeze(0)[1:]         # [seq_len-1]

        all_activations.append(act_no_bos)
        all_tokens.append(tok_no_bos)
        bos_count += 1

    print(f"  (已排除 {bos_count} 个 BOS token)")
...
def validate_reconstruction(sae, all_activations):
    ...
    print(f"\n  总 token 数 (排除 BOS): {activations.shape[0]}")
    print(f"  MSE:               {mse:.6f}")
...
def validate_interpretability(model, sae, json_texts, natural_texts, hook_name):
    ...
    print("  （已排除 BOS token，避免 BOS 高激活霸占排序）")
    print("=" * 60)
    ...
    for label, texts in [("JSON", json_texts), ("自然语言", natural_texts)]:
        ...
        for text in texts:
            ...
            with torch.no_grad():
                feature_acts = sae.encode(activations)  # [seq_len-1, d_sae]

            # 解码 token 为字符串（同样跳过 BOS）
            token_ids = tokens.squeeze(0)[1:]
    ...
    print(f"\n{'─' * 50}")
    print(f"  JSON 结构特征分析（排除 BOS）")
    print(f"{'─' * 50}")
    print(f"  寻找在 JSON 标点 ({{ }} [ ] \" : ,) 上特别活跃的特征...\n")

    json_punct_chars = {'{', '}', '[', ']', '"', ':', ','}

    # 获取 JSON 文本的特征激活和 token（排除 BOS）
    json_feature_acts = []
    json_token_strs = []
    json_is_punct = []
