...
- ✅ Mistral-7B-Instruct-v0.3 三层 SAE 全部训练完成并验证通过
- ⏳ 上传 HuggingFace
- ⏳ Llama-3.1-8B-Instruct 三层 SAE（下一步）
...
| `sae_checkpoints/mistral7b_sae_L22_64k/` | **Layer 22 SAE 权重（v0.3 + lmsys）✅** |

### 下一步

- 上传 HuggingFace（三层一起传）
- 训 Llama-3.1-8B-Instruct 三层（同样 L8/L16/L22，lmsys chat-formatted）
- ZL 同步推 format specificity 对照实验
