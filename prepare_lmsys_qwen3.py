"""
预处理 lmsys-chat-1m-chat-formatted 数据集（Qwen3 版）
用 Qwen3 的 chat template 渲染，关闭 thinking 模式

用法（在容器 d2l_exp 里跑）：
  python prepare_lmsys_qwen3.py
"""
...
OUTPUT_DIR = "/workspace/datasets/lmsys_chat_qwen3"
    ...
    def render_chat(example):
        ...
        try:
            text = tokenizer.apply_chat_template(
                conv,
                tokenize=False,
                enable_thinking=False,  # 关掉 thinking 模式
            )
        except Exception:
            text = ""
        return {"text": text}

    print("渲染 chat template (enable_thinking=False)...")
